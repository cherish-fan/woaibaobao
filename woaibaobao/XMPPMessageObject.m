...
@implementation XMPPMessageObject
...
-(void)dealloc
{
    NSLog(@"XMPPMessageObject.delloc");
...
-(NSMutableDictionary*)toDictionary
{
    ...
    [_dictionary setValue:_fromUserId forKey:kMESSAGE_FROM];
    [_dictionary setValue:_toUserId forKey:kMESSAGE_TO];
    [_dictionary setValue:_content forKey:kMESSAGE_CONTENT];
    [_dictionary setValue:[f stringFromDate:_timeSend] forKey:kMESSAGE_TIMESEND];
    //    [dictionary setValue:[f stringFromDate:timeReceive] forKey:kMESSAGE_TIMERECEIVE];
    [_dictionary setValue:_type forKey:kMESSAGE_TYPE];
    [_dictionary setValue:[_fileData base64EncodedString] forKey:kMESSAGE_FILEDATA];
    [_dictionary setValue:_fileName forKey:kMESSAGE_FILENAME];
    [_dictionary setValue:_fileSize forKey:kMESSAGE_FILESIZE];
    [_dictionary setValue:_location_x forKey:kMESSAGE_LOCATION_X];
    [_dictionary setValue:_location_y forKey:kMESSAGE_LOCATION_Y];
    [_dictionary setValue:_timeLen forKey:kMESSAGE_TIMELEN];
...
-(BOOL)saveMsg:(NSString*)dbName tableName:(NSString*)tableName{
    if([dbName length]<=0)
        return NO;
    FMDatabase* db = [[XMPPUtils sharedInstance] openUserDb:dbName];
    ...
    worked = [db executeUpdate:insertStr,self.fromUserId,self.toUserId,self.content,self.type,self.messageId,self.timeSend,self.timeReceive,[self.fileData base64EncodedString],self.fileName,self.fileSize,self.location_x,self.location_y,self.timeLen,self.isRead,self.isSend];
    //    FMDBQuickCheck(worked);
...
-(BOOL)updateIsRead:(BOOL)b{
    ...
    FMDatabase* db = [[XMPPUtils sharedInstance] openUserDb:toUserId1];
...
+(void)fromRs:(XMPPMessageObject *)p rs:(FMResultSet*)rs{
...
+(NSMutableArray*)fetchMessageListWithUser:(NSString *)userId byPage:(int)pageIndex
{
    ...
    while ([rs next]) {
        XMPPMessageObject *p=[[XMPPMessageObject alloc]init];
        [XMPPMessageObject fromRs:p rs:rs];
...
+(NSMutableArray *)fetchRecentChatByPage:(int)pageIndex
{
    ...
    FMDatabase* db = [[XMPPUtils sharedInstance] openUserDb:userId];
