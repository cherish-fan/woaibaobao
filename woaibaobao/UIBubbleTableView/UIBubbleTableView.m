...
- (void)initializator
{
    ...
    UIMenuItem *testMenuItem = [[UIMenuItem alloc] initWithTitle:@"分享" action:@selector(share:)];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 0) {
        ...
        for (NSInteger i = 0; i < section; i++) {
            sumRow += [self numberOfRowsInSection:i];
        }
        
        NSInteger switchedRow = sumRow - section + row - 1;
        
        NSLog(@"index.section:%ld row:%ld",indexPath.section,indexPath.row);
        NSLog(@"switchedrow:%ld", switchedRow);
        
        
        
        [self.bubbleDataSource didSelectBubbleRowAtIndexRow:switchedRow];
    }
